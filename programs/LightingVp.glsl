...
#endif
	vec4 positionEyeSpace = gl_ModelViewMatrix * position;
	// Pass-through default components
	gl_Position    = gl_ProjectionMatrix * positionEyeSpace;
...
// TODO SK: rewrite without using built-in constants, reduce redundancy
	...
	if (numberOfActiveLights > 0.0) {
		...
		vec3 L = gl_LightSource[0].position.xyz - positionEyeSpace.xyz * gl_LightSource[0].position.w;
	...
	if (numberOfActiveLights > 1.0) {
		// Compute vector from surface to light position
		vec3 L = gl_LightSource[1].position.xyz - positionEyeSpace.xyz * gl_LightSource[1].position.w;
	...
	if (numberOfActiveLights > 2.0) {
		// Compute vector from surface to light position
		vec3 L = gl_LightSource[2].position.xyz - positionEyeSpace.xyz * gl_LightSource[2].position.w;
